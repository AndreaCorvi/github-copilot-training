...
async def log_task(task: DeveloperTask) -> dict[str, str | int]:
    ...
    Assigns a new unique ID to the provided task, stores it in the in-memory
    task store, and returns a confirmation message with the assigned task ID.

    Args:
        task: The DeveloperTask to be logged.

    Returns:
        A dict containing a confirmation message and the assigned task_id.
    """
    new_id = max(MOCK_TASKS.keys()) + 1 if MOCK_TASKS else 1
    task.task_id = new_id
    MOCK_TASKS[new_id] = task
    
    return {"message": "Task logged successfully.", "task_id": task.task_id}
